...
template<typename T, int rawSize>
myFIFO<T,rawSize>::myFIFO() : size(rawSize) {
	flush();
}
template<typename T, int rawSize>
bool myFIFO<T,rawSize>::enqueue( T element ) {
...
template<typename T, int rawSize>
T myFIFO<T,rawSize>::dequeue() {
	numberOfElements--;
	nextOut %= size;
	return raw[ nextOut++];
}
template<typename T, int rawSize>
T myFIFO<T,rawSize>::peek() const {
	return raw[ nextOut % size];
}
template<typename T, int rawSize>
void myFIFO<T,rawSize>::flush() {
